...
times = pd.to_datetime(ds_hourly.variables['time'][:]) + timedelta(minutes=2) # Add 2 minutes again to get correct times
year = times.year
...
total_flashes_2015 = np.add(F_CC_summer_2015, F_CG_summer_2015)

## Create netcdf file to store diurnal data (length of time vector = 24)
ds_obs = Dataset('/data/leuven/336/vsc33651/projects/nu-wrf-dev/Lightning_data/Observations_diurnal.nc', mode='w', format='NETCDF4')
...
for i in range(0,24):
    ...
    ds_obs['LF_diurnal'][i, :, :] = np.mean(total_flashes_2015[time_select_vector, :, :], axis=0)
ds_obs.close()