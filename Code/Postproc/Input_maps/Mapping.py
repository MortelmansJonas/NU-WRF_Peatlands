...
inds_lat_peat = np.unique(np.where((PEATMAP['lat'][:] > 58) & (PEATMAP['lat'][:] <63))[0])
inds_lon_peat = np.unique(np.where((PEATMAP['lon'][:] > -124) & (PEATMAP['lon'][:] <-108))[0])
lat_in = PEATMAP['lat'][inds_lat_peat]
lon_in = PEATMAP['lon'][inds_lon_peat]

lon_peat, lat_peat = np.meshgrid(lon_in, lat_in)


## Get only the study area
inds_lat = np.unique(np.where((LIS_2015['lat'][:] > 58) & (LIS_2015['lat'][:] < 62))[0])
inds_lon = np.unique(np.where((LIS_2015['lon'][:] > -123) & (LIS_2015['lon'][:] < -108))[1])

Topo = met['HGT_M'][0,inds_lat, inds_lon] # Height above MSL (in meters)
LU = met['LU_INDEX'][0,inds_lat, inds_lon] # Land Use (categorical 0-21)
print(np.unique(LU))

Albedo = np.zeros((2160, 170,309))
Albedo[0:360,:,:] = LIS_2015['Albedo'][:,inds_lat, inds_lon]
Albedo[360:720,:,:] = LIS_2016['Albedo'][:,inds_lat, inds_lon]
Albedo[720:1080,:,:] = LIS_2017['Albedo'][:,inds_lat, inds_lon]
Albedo[1080:1440,:,:] = LIS_2018['Albedo'][:,inds_lat, inds_lon]
Albedo[1440:1800,:,:] = LIS_2019['Albedo'][:,inds_lat, inds_lon]
Albedo[1800:2160,:,:] = LIS_2020['Albedo'][:,inds_lat, inds_lon]

Albedo[Albedo>1] = np.nan # To drop fill value over lake and water

GF = np.zeros((2160, 170,309))
GF[0:360,:,:] = LIS_2015['GF'][:,inds_lat, inds_lon]
GF[360:720,:,:] = LIS_2016['GF'][:,inds_lat, inds_lon]
GF[720:1080,:,:] = LIS_2017['GF'][:,inds_lat, inds_lon]
GF[1080:1440,:,:] = LIS_2018['GF'][:,inds_lat, inds_lon]
GF[1440:1800,:,:] = LIS_2019['GF'][:,inds_lat, inds_lon]
GF[1800:2160,:,:] = LIS_2020['GF'][:,inds_lat, inds_lon]

GF[GF>1] = np.nan # To drop fill value over lake and water

print(np.nanmax(Albedo))
print(np.nanmax(GF))
# ---------------------------------------------------------------------------------------------
# MAP
# ---------------------------------------------------------------------------------------------
...
# 9=Savannah, 10=Grassland, 11=Permanents Wetland, 12=Cropland, 13=Urban and Built-up, 14=Cropland/Natural Mosaic
# 15=Snow and Ice, 16=Barren or Sparsely Vegetated, 17=Water, 18=Wooded Tundra, 19=Mixed Tundra
# 20=Barren Tundra, 21=Lake
# The area only contains: [ 1.  3.  4.  5.  6.  7. 10. 11. 12. 13. 16. 18. 19. 21.]

LU_cmap_data = ['tab:green', 'white', 'tab:olive', 'tab:brown', 'black', 'lightsteelblue', 'yellow', 'tab:cyan',
                'tab:orange', 'tab:red', 'tab:gray', 'tab:purple', 'tab:pink', 'tab:blue']
LU_cmap = ListedColormap(LU_cmap_data)

# Rescale LU to eliminate the missing values (the LU that are not present)
LU[LU==3] = 2
LU[LU==4] = 3
LU[LU==5] = 4
LU[LU==6] = 5
...
m2 = Basemap(projection= 'lcc', lat_0 = lat.mean(),
            lon_0 = lon.mean(),
            llcrnrlon=(-123), llcrnrlat=(58),
            urcrnrlon=(-108),urcrnrlat = (62),
...
m5.drawmapscale(-121, 58.7, lon_peat.mean(), lat_peat.mean(), 100)
...
map5= m5.pcolormesh(lon_peat,lat_peat,PEATMAP['Band1'][inds_lat_peat,inds_lon_peat],latlon=True,cmap='YlGn')
plt.title('Peat fraction', fontsize=font_title)
cbar = plt.colorbar(map5, shrink = 0.8)
cbar.ax.tick_params(labelsize=font_ticks)
plt.show()
