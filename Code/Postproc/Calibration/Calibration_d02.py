...
ds_obs = Dataset('/scratch/leuven/projects/lt1_2020_es_pilot/project_output/rsda/vsc33651/NU-WRF/wrfout_nc_files/CLDN_at_domain2_all.nc')
...
c2 = np.where(LTG3_sorted_d02==0)[0][0]
...
# Cut obs if needed
c3 = np.nanmin(np.array([Obs.shape[0],LTG3_c_d02.shape[0]]))
reg_LTG3_d02 = lr().fit(LTG3_c_d02.reshape((-1,1)), Obs[0:c3])
...
while sum_lpi_d02 < Tot_flashes:
    ...
    LPI_cutoff_d02 = LPI_adj_sorted_d02[i]
    LPI_adj_sorted_d02.reshape((13248, 170, 309))
i=0
while (sum_ltg3_d02 < Tot_flashes) & (i < len(LTG3_adj_sorted_d02)):
    sum_ltg3_d02 = sum_ltg3_d02 + LTG3_adj_sorted_d02[i]
    i += 1
else:
    if (i >= len(LTG3_adj_sorted_d02)):
        LTG3_adj_sorted_d02.reshape((13248, 170, 309))
    else:
        LTG3_adj_sorted_d02[i:] = 0
        LTG3_cutoff_d02 = LTG3_adj_sorted_d02[i]
        LTG3_adj_sorted_d02.reshape((13248, 170, 309))
i=0
...
while sum_capexp_r_d02 < Tot_flashes:
    ...
    CAPExP_R_cutoff_d02 = CAPExP_R_adj_sorted_d02[i]
    CAPExP_R_adj_sorted_d02.reshape((13248, 170, 309))

LPI_d02_adj[LPI_d02_adj<=LPI_cutoff_d02] == 0
...
# ---------------------------------------------------------------------------------------------
# Create .nc file
ds = Dataset('/scratch/leuven/projects/lt1_2020_es_pilot/project_output/rsda/vsc33651/NU-WRF/wrfout_nc_files/data_calibrated_ax_d02.nc', mode='w', format='NETCDF4')
